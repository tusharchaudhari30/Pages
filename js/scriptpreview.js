const imageParser = (block) => {
  ...
  <p class=" text-xl text-center">${block.data.caption}</p>
  `
};
var edjsHTML = (function () {
  ...
  return (r = {}) => (
    ...
    {
      parse: ({ blocks: r }) =>
        r.map((r) => (e[r.type] ? e[r.type](r) : t(r.type))),
      parseBlock: (r) => (e[r.type] ? e[r.type](r) : t(r.type)),
    }
  );
})();
...
const cleanData = {
  blocks: [
    ...
    },
    {
      ...
      data: {
        url:
          "https://scottraymondsite.files.wordpress.com/2017/01/wind-9.jpg",
...
let html = edjsParser.parse(cleanData);
let preview = document.getElementById("preview");
html.map(a=>{
  preview.innerHTML=preview.innerHTML+a;
})
